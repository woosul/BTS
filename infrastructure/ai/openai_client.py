...
class OpenAIClient(BaseAIClient):
    ...
    def __init__(
        ...
        Args:
            ...
            model: OpenAI 모델 (None이면 settings에서 로드)
        ...
        if not self.api_key:
            raise ValueError(
                ...
                ".env 파일에 OPENAI_API_KEY를 설정하거나 인자로 전달하세요."
            )
            ...
            try:
                ...
                response = self.client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "당신은 암호화폐 전문 트레이더입니다. 반드시 JSON 형식으로만 답변하세요."
                        },
                        {
                            "role": "user",
                            "content": prompt
                ...
                logger.error(f"OpenAI API 호출 실패 ({model}): {e}")
                if idx == len(models_to_try) - 1:
                    # 모든 모델 시도 실패
                    return {
                        "recommendation": "hold",
    ...
    def _build_entry_prompt(
        ...
        """매수 평가 프롬프트 생성"""
        return f"""다음 데이터를 분석하여 매수 시그널을 평가해주세요.
...
"""
    ...
    def _build_exit_prompt(
        ...
        """매도 평가 프롬프트 생성"""
        return f"""다음 보유 포지션의 매도 시점을 평가해주세요.
...
"""
    ...
    def __repr__(self) -> str:
        return f"<OpenAIClient(model={self.model}, max_tokens={self.max_tokens})>"
